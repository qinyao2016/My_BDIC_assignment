...
public class AdjacencyMatrixTest {
   public static void printMatrix(IGraph<String,Integer> g,String action,String variable) {
	   if(action == "i")
	   ...
	   System.out.print("    ");
	   IIterator<IVertex<String>> vertices = g.vertices();
	   while(vertices.hasNext()) {
		   System.out.print(vertices.next().element() + " ");
	   }
	   System.out.println("\n");
	   vertices = g.vertices();
	   IEdge<Integer>[][] matrix = ((AdjacencyMatrixGraph)g).matrix();
	      for(int i = 0;i < ((AdjacencyMatrixGraph)g).count();i++) {
			   System.out.print(vertices.next().element() + " ");
	    	  		for(int j = 0;j < ((AdjacencyMatrixGraph)g).count();j++) {
	    	  			if(matrix[i][j] != null)
